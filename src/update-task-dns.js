...
exports.handler = async (event, context, callback) => {
  ...
  console.log(
    `cluster: ${clusterName}, domain: ${domain}, hostedZone: ${hostedZoneId}`
  )

  if (!domain || !hostedZoneId) {
    console.log(
      `Skipping. Reason: no "domain" and/or "hostedZoneId" tags found for cluster ${clusterArn}`
    )
    return
  }

  const taskDeftags = await fetchTaskDefinitionTags(task.taskDefinitionArn)
  console.log(`taskDefTags[${task.taskDefinitionArn}]: - ${taskDeftags}`)
  const setWildcard = taskDeftags['setWildcard'] === 'true'
  const setDNS = taskDeftags['setDNS'] === 'true'
  const dnsPrefix = taskDeftags['dnsPrefix']
  ...
  const serviceName = dnsPrefix || task.group.split(':')[1]
  ...
  if (setWildcard) {
    const wildCardContainerDomain = `*.${serviceName}.${domain}`
    const wildCardRecordSet = createRecordSet(
      wildCardContainerDomain,
      taskPublicIp
    )

    await updateDnsRecord(clusterName, hostedZoneId, wildCardRecordSet)
  }

  console.log(
    `DNS record update finished for ${containerDomain} (${taskPublicIp})`
  )
}
...
async function fetchTaskDefinitionTags(taskDefinitionArn) {
  const response = await ecs
    .listTagsForResource({
